const UserValidation = require("./user.validation");
const DepartmentValidation = require("./department.validation");
const ProjectValidation = require("./project.validation");
const RoleValidation = require("./role.validation");
const UsersProjectsRolesValidation = require("./usersProjectsRoles.validation");
const RolesProjectsValidation = require("./rolesProjects.validation");
const PeriodValidation = require("./period.validation");
const ObjectiveValidation = require("./objective.validation");
const TaskTypeValidation = require("./taskType.validation");

module.exports = {
  UserValidation,
  DepartmentValidation,
  ProjectValidation,
  RoleValidation,
  UsersProjectsRolesValidation,
  RolesProjectsValidation,
  PeriodValidation,
  ObjectiveValidation,
  TaskTypeValidation,
};
